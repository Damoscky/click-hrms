            ...
            <div class="row staff-grid-row">
                @if (count($totalEmployees) > 0)
                    @foreach ($totalEmployees as $employee)
                        <div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3 profile-widget" data-employeeId="{{ $employee->employeeRecord->employee_id }}" data-employeeDepartment="{{ $employee->employeeRecord->department->name }}">
                            <div class="profile-img">
                                @if (isset($employee->employeeRecord->image))
                                    <a href="{{ route('admin.employee.show', base64_encode($employee->id)) }}" class="avatar"><img src="{{ $employee->employeeRecord->image }}" alt="Profile Picture" /></a>
                                @else
                                    <a href="{{ route('admin.employee.show', base64_encode($employee->id)) }}" class="avatar"><img src="{{ asset('assets') }}/img/user.png" alt="User Image" /></a>
                                @endif
                            </div>
                            ...
                            <a href="{{route('admin.employee.resend.notification', base64_encode($employee->id) )}}" class="btn btn-white btn-sm m-t-10">Resend Notification</a>
                        </div>
                    @endforeach
                @else
